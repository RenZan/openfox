...
describe('ws/protocol', () => {
  ...
  describe('session and project messages', () => {
    ...
    it('builds project and session list messages', () => {
      ...
      expect(createSessionListMessage([{ id: 'session-1', projectId: 'project-1', workdir: '/tmp/project', phase: 'plan', isRunning: false, createdAt: 'a', updatedAt: 'b', criteriaCount: 0, criteriaCompleted: 0, messageCount: 0 }], 'corr')).toEqual({
        id: 'corr',
        type: 'session.list',
        payload: { sessions: [{ id: 'session-1', projectId: 'project-1', workdir: '/tmp/project', phase: 'plan', isRunning: false, createdAt: 'a', updatedAt: 'b', criteriaCount: 0, criteriaCompleted: 0, messageCount: 0 }] },
      })
  ...
  describe('storedEventToServerMessage', () => {
    ...
    it('converts streamable events to websocket messages and skips internal ones', () => {
      const events: StoredEvent[] = [
        ...
        { ...baseEvent, type: 'turn.snapshot', data: { phase: 'plan', isRunning: false, messages: [], criteria: [], contextState: { currentTokens: 0, maxTokens: 1, dangerZone: false, canCompact: false, compactionCount: 0 }, currentContextWindowId: 'window-1', todos: [], readFiles: [], snapshotSeq: 1, snapshotAt: 1 } },
      ...
      expect(converted[9]).toEqual({ type: 'session.running', payload: { isRunning: true } })
      expect(converted[10]).toEqual({ type: 'criteria.updated', payload: { criteria: [] } })
      expect(converted[11]).toBeNull()
      expect(converted[12]).toEqual({ type: 'context.state', payload: { context: { currentTokens: 1, maxTokens: 2, dangerZone: false, canCompact: true, compactionCount: 0 } } })
      expect(converted[13]).toEqual({ type: 'chat.todo', payload: { todos: [{ content: 'write tests', status: 'pending' }] } })
      expect(converted[14]).toEqual({ type: 'chat.done', payload: { messageId: 'm1', reason: 'complete' } })
      expect(converted[15]).toEqual({ type: 'chat.error', payload: { error: 'boom', recoverable: false } })
      expect(converted[16]).toEqual({ type: 'chat.format_retry', payload: { attempt: 2, maxAttempts: 10 } })
      expect(converted[17]).toBeNull()
