...
describe('Tool Preparing Events', () => {
  ...
  it('emits chat.tool_preparing before chat.tool_call for read_file', async () => {
    ...
    const preparingIdx = allEvents.findIndex(e => 
      e.type === 'chat.tool_preparing' && (e.payload as ChatToolPreparingPayload).name === 'read_file'
    )
    const toolCallIdx = allEvents.findIndex(e => 
      e.type === 'chat.tool_call' && (e.payload as ChatToolCallPayload).tool === 'read_file'
    )
  ...
  it('emits preparing events for multiple tool calls', async () => {
    ...
    for (const tcEvent of toolCallEvents) {
      const matchingPreparing = preparingEvents.find(pe => 
        pe.payload.name === tcEvent.payload.tool
      )
